...
{ 
  ...
  else {print("ERREUR: type not expected")}  # erreur affichée si la condition est disrespectée
}
...
brutToNet2 <- function(salaire_brut, type_sal) #création de la fonction qui s'appelle 'brutToNet2 qui prend en entrée deux arguments: le salaire brut et le type de salarié
{
  ...
  if (is.numeric(salaire_brut) & type_sal %in% c("C", "N")) # condition pour que les arguments saisis soient du bon type
  {
    ...
    return (salaire_net)}
  } # calcul effectué si la condition est respecté et renvoie le résultat souhaité
  
  else {print("ERREUR. Contract unknown")} # erreur affichée si la condition est disrespectée
}

# 2.1.3
brutToNet3 <- function(salaire_brut, type_sal, tauxpr, tempstr) #création de la fonction qui s'appelle 'brutToNet2 qui prend en entrée quatre arguments: le salaire brut, le type de salrié (obligatoires), le taux de prélèvement, le temps de travail (facultatifs)
{
  ...
  else {print("ERREUR: contract unkown")} # erreur affichée si la condition est respecté et arrête le programme
  ...
  else {print("ERREUR: rate must be in range (0;100)")} # erreur affichée si la condition est respecté et arrête le programme
  
  maliste <- list(paste("Salaire net avant impôt:" ,salaire_netav, "€"),
                  paste ("Salaire net après impôt:",salaire_netap, "€")) # créationd d'une liste permettant de stocker les deux résultat obtenus pour renvoyer 
